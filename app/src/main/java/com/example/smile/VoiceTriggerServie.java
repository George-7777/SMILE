...
public class VoiceTriggerServie extends Service {
    private SpeechRecognizer speechRecognizer;
    private Intent recognizerIntent;
    private boolean isListening = false;
    private String triggerPhrase = "смайлик";

    // Константа для действия Broadcast
...
//        audio.setStreamVolume(AudioManager.STREAM_MUSIC, 0, AudioManager.FLAG_REMOVE_SOUND_AND_VIBRATE);
    ...
    private void onTriggerDetected(String spokenText) {
        ...
        // 1. Останавливаем фоновое прослушивание
        stopListening();

        // 2. Отправляем Broadcast
        sendTriggerBroadcast(spokenText);

        // 3. Воспроизводим звуковой сигнал (опционально)
        //playNotificationSound();

        // 4. Ждем 5 секунд и возобновляем прослушивание
        new Handler(Looper.getMainLooper()).postDelayed(() -> {
            startListening();
        }, 5000);
    }
...
//        }
    ...
    public static void startService(Context context) {
        Intent serviceIntent = new Intent(context, VoiceTriggerServie.class);
